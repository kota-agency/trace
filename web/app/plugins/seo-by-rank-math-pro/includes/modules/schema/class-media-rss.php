...
class Media_RSS {
	...
	public function add_video_data() {
		...
		$url         = ! empty( $video_schema['contentUrl'] ) ? $video_schema['contentUrl'] : $video_schema['embedUrl'];
		$width       = ! empty( $video_schema['width'] ) ? $video_schema['width'] : '';
		$height      = ! empty( $video_schema['height'] ) ? $video_schema['height'] : '';
		...
		$this->newline( '<media:content url="' . esc_url( $url ) . '" medium="video" height="' . esc_attr( $height ) . '" width="' . esc_attr( $width ) . '">' );
