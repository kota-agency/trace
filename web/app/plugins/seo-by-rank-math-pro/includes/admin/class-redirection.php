<?php
/**
 * Pro Redirection admin features.
 *
 * @since      1.0
 * @package    RankMathPro
...
class Redirection {
	...
	public function maybe_sync_htaccess() {
		...
		if ( $htaccess === false || ! $htaccess['writable'] || get_filesystem_method() !== 'direct' ) {
	...
	public function admin_scripts( $hook ) {
		...
		wp_enqueue_style( 'rank-math-pro-redirections', RANK_MATH_PRO_URL . 'assets/admin/css/redirections.css', [], RANK_MATH_PRO_VERSION );
		wp_enqueue_script( 'rank-math-pro-redirections', RANK_MATH_PRO_URL . 'assets/admin/js/redirections.js', [], RANK_MATH_PRO_VERSION, true );
	}
	...
	public function get_redirections_query( $table, $args ) {
		...
		$table->whereIn( $wpdb->term_relationships . '.term_taxonomy_id', $categories );
	}
	...
	public function back_to_redirections_link( $taxonomy ) {
		...
		echo '<p><a href="' . esc_url( $link ) . '">' . esc_html__( '&larr; Go Back to the Redirections' ) . '</a></p>';
	}
