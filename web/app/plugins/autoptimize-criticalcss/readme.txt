...
= I only see (one "P" and multiple) "N" jobs and no rules and I'm getting "WordPress cron"-warnings, what should I do? =

If all jobs remain in "N" (and one `is_front_page` job is "P") then wordpress "cron job" that does the queue processing is not getting triggered. To verify you can install the ["wp crontrol"-plugin](https://wordpress.org/plugins/wp-crontrol/) and then under Tools -> Cron Events look for "ao_ccss_queue" and check the "next run" time/ date.
...
= When "inline & defer CSS" is active, the full CSS isn't loaded any more! =

This is caused by mod_pagespeed, which -wrongly- assumes the full CSS is not needed any more. You will have to reconfigure mod_pagespeed to prevent this from happening, [the solution in this forum-reply](https://wordpress.org/support/topic/only-critical-css-is-loaded-but-not-the-original-css/#post-12642096) might help.
...
* preparing for integration in AO proper: changing file layout and moving to OO.
